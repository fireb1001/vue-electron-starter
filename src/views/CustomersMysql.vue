...
import { knex, moment, mysqlknex } from '../main'
...
export default {
  ...
  methods: {
      ...
      if(this.flags.show_g_than){
        ...
        this.customers_arr = await new CustomersCtrl().findAll(
          {debt_g_than: this.flags.show_g_than},
          {
            ...
            orderByTransDay: this.app_config.shader_name == 'mmn1' 
          }
        )
    ...
    async show_all_cust(){
      ...
      */
      let results = await mysqlknex.raw('select * from customers');
      this.customers_arr =  results[0].map(each => new CustomerDAO(each))
      //console.log(results)
    },
    fresh_form(){
